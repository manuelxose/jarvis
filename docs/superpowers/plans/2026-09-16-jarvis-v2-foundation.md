...
- [ ] **Step 3: Refresh the project graph.**
  ...
  Run `graphify update .` from `/home/user/workspace/jarvis`. Confirm the command completes and that `graphify-out/graph.json` contains the new `jarvis.apps`, `jarvis.core`, `jarvis.config`, and `jarvis.observability` nodes. Do not treat graph output as test evidence.
